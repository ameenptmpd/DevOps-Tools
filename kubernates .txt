...
3. sudo yum clean all && sudo yum -y makecache
4. sudo yum -y install epel-release vim git curl wget kubelet kubeadm kubectl --disableexcludes=kubernetes
...
15. Ensure all required firewalls are open between all the different nodes. (not mandatory)
master
